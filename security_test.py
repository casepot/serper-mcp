...
class SecurityTester:
    ...
    async def test_no_authentication(self):
        ...
        try:
            ...
            self.log_test_result(
                "No Authentication", False, f"Exception during test: {str(e)}"
            )
    ...
    async def test_invalid_token(self):
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 401:
                    self.log_test_result(
                        "Invalid Token", True, "Correctly rejected invalid token"
                    )
                else:
                    self.log_test_result(
                        "Invalid Token",
            ...
            self.log_test_result(
                "Invalid Token", False, f"Exception during test: {str(e)}"
            )
    ...
    async def test_valid_authentication(self):
        ...
        try:
            client = Client(self.sse_url, auth=BearerAuth(token=TEST_TOKEN))
            ...
            if "scope" in str(e).lower():
                ...
                self.log_test_result(
                    "Valid Authentication", False, f"Tool error: {str(e)}"
                )
        except Exception as e:
            self.log_test_result(
                "Valid Authentication", False, f"Exception during test: {str(e)}"
            )
    ...
    async def test_input_validation_query_length(self):
        ...
        if not TEST_TOKEN:
            self.log_test_result(
                "Query Length Validation", False, "No test token provided"
            )
            return
        ...
        try:
            ...
            async with client:
                ...
                self.log_test_result(
                    "Query Length Validation", False, "Long query was not rejected"
                )
        except ToolError as e:
            if "too long" in str(e).lower() or "length" in str(e).lower():
                ...
                self.log_test_result(
                    "Query Length Validation", False, f"Unexpected error: {str(e)}"
                )
        except Exception as e:
            self.log_test_result(
                "Query Length Validation", False, f"Exception during test: {str(e)}"
            )
    ...
    async def test_input_validation_malicious_content(self):
        ...
        if not TEST_TOKEN:
            self.log_test_result(
                "Malicious Content Validation", False, "No test token provided"
            )
            return
    ...
    async def test_rate_limiting(self):
        ...
        if not TEST_TOKEN:
            self.log_test_result("Rate Limiting", False, "No test token provided")
            return
        ...
        try:
            ...
            self.log_test_result(
                "Rate Limiting", False, f"Exception during test: {str(e)}"
            )
    ...
    async def test_parameter_validation(self):
        ...
        if not TEST_TOKEN:
            self.log_test_result(
                "Parameter Validation", False, "No test token provided"
            )
            return
        ...
        try:
            ...
            async with client:
                try:
                    await client.call_tool(
                        "google_search", {"query": "test", "num_results": 200}
                    )
                except ToolError as e:
                ...
                try:
                    await client.call_tool(
                        "google_search", {"query": "test", "page_number": 50}
                    )
                except ToolError as e:
            ...
            if validation_errors > 0:
                ...
                self.log_test_result(
                    "Parameter Validation", False, "No parameter validation detected"
                )
        except Exception as e:
            self.log_test_result(
                "Parameter Validation", False, f"Exception during test: {str(e)}"
            )
    ...
    async def test_error_information_disclosure(self):
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                disclosed_patterns = [
                    p for p in sensitive_patterns if p in response_text
                ]
...
async def main():
    ...
    if not TEST_TOKEN:
        print(
            "⚠️  WARNING: No TEST_TOKEN provided. Authentication tests will be limited."
        )
